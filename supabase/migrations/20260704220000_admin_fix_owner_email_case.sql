-- Fix: a exclusão da conta do dono comparava e-mail com case-sensitivity
-- (IS DISTINCT FROM sem lower()), então não batia e a conta lifetime
-- continuava contada em "assinantes ativos". Emails são case-insensitive
-- por convenção (mesmo padrão já usado em cakto-webhook).
