...
export const PedirLembretes = ({ dueDays }: { dueDays: { day?: number; bills?: { paid?: boolean }[] }[] }) => {
  ...
  const ligar = async () => {
    ...
    if (ok) {
      // Agenda TUDO o que estiver ligado nas preferências, não só as contas:
      // a permissão do Android é uma só, e quem acabou de aceitar não vai
      // querer ser perguntado de novo por causa da retrospectiva. Desligar
      // cada tipo é papel da central, no menu.
      const { reagendarTudo } = await import("@/lib/reagendar");
      const contagem = await reagendarTudo(get, lerPrefs(get<unknown>(CHAVE_PREFS, undefined)));
      trackEvent("lembretes_agendados", contagem);
      setPronto(true);
