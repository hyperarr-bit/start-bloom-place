...
import { RefreshCw, ArrowDownRight, Eye, MousePointerClick, UserPlus, CreditCard } from "lucide-react";

type CtaRow = { cta: string; clicks: number; sessions: number };
type SourceRow = { source: string; visits: number };
type DailyRow = { day: string; visits: number; cta_clicks: number; signups: number };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Funil LP → Pagante</h1>
          <p className="text-xs text-zinc-500 mt-1">
            Landing page real (/lp): visita → CTA → cadastro → pagante · sessões únicas.
            O tutorial do quickstart vive na aba Onboarding.
          </p>
        </div>
                ...
                <table className="w-full text-xs">
                  <thead className="text-zinc-500 uppercase text-[10px] tracking-wider">
                    <tr>
                      ...
                      <th className="text-left px-3 py-2 w-40">Visita → Cadastro</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.daily.map((r, i) => {
                      const conv = r.visits > 0 ? (r.signups / r.visits) * 100 : 0;
                      return (
                        <tr key={i} className="border-t border-zinc-800/50">
