...
/**
 * Webhook da PAGAR.ME (21/07) — gêmeo do abacate-webhook para o braço B do
 * teste A/B de gateway. Fecha o "pagou e não liberou" de quem paga no app do
 * banco e não volta pra tela (o polling do app nunca confirma).
 *
 * SEGURANÇA (dupla trava — a Pagar.me v5 NÃO assina o payload, ao contrário
 * da v3 legada que usava X-Hub-Signature):
 *   1. secret na query (?webhookSecret=) — o painel deles não tem campo de
 *      secret, mas aceita a URL completa; opcionalmente dá pra somar Basic
 *      Auth no painel.
 *   2. RECONFIRMA o pedido na API da Pagar.me antes de liberar. Nunca confia
 *      no corpo do webhook (à prova de payload forjado).
 *
 * Eventos: order.paid (e charge.paid como redundância idempotente).
 * Sempre responde 2xx rápido — erro nosso não pode queimar as tentativas de
 * reenvio deles.
 */

const PAGARME_API = "https://api.pagar.me/core/v5";
const PRECOS_CENTAVOS: Record<string, number> = { lifetime: 9790, downsell: 1490, w97: 9790, w47: 4790, w27: 2790 };
...
async function sendToUtmify(admin: Admin, args: { userId: string; email: string | null; orderId: string; offer: string; amountCents: number; name: string | null }) {
  ...
  try {
    ...
    const payload = {
      orderId: args.orderId, platform: "Pagarme", paymentMethod: "pix", status: "paid",
...
/**
 * CAPI direta do Meta. SEM IP/UA aqui (quem chama é a Pagar.me, não o
...
serve(async (req) => {
  ...
  if (!secret || !apiKey) { logStep("Missing PAGARME_WEBHOOK_SECRET/API_KEY"); return new Response("Server misconfiguration", { status: 500 }); }
  ...
  try {
    ...
    // order.paid → data é o pedido (or_...). charge.paid → data é a cobrança,
    // e o pedido vem em data.order.id. Só sabemos consultar PEDIDO.
    const orderId = String(
      (typeof data.id === "string" && data.id.startsWith("or_") ? data.id : "") ||
      data.order?.id || data.order_id || "",
    );
    logStep("Webhook received", { event, orderId: orderId.slice(0, 24) });
    if (!orderId) return jsonResponse({ received: true, note: "no_order_id" });

    // idempotência por (evento, pedido)
    const { error: dupErr } = await admin.from("webhook_events").insert({ id: `${event}:${orderId}`, source: "pagarme", event });
    if (dupErr && (dupErr as { code?: string }).code === "23505") {
      logStep("Duplicate ignored", { event, orderId: orderId.slice(0, 24) });
      return jsonResponse({ received: true, duplicate: true });
    }

    // TRAVA 2: reconfirma o pagamento na API (nunca confia só no payload)
    const chk = await fetch(`${PAGARME_API}/orders/${encodeURIComponent(orderId)}`, { headers: { Authorization: basicAuth(apiKey) } });
    const chkOut = await chk.json().catch(() => ({}));
    const status = String(chkOut?.status ?? "unknown").toUpperCase();
    if (status !== "PAID") { logStep("Not paid — ignoring", { orderId: orderId.slice(0, 24), status }); return jsonResponse({ received: true, status }); }

    // resolve dono + oferta: 1º pix_order_created (server), 2º pix_generated (cliente), 3º metadata do pedido
    let userId: string | null = null;
    let offer = "";
    const { data: reg } = await admin.from("analytics_events").select("user_id, event_data").eq("event_name", "pix_order_created").contains("event_data", { order_id: orderId }).limit(1);
    if (reg?.[0]) { userId = reg[0].user_id as string; offer = String((reg[0].event_data as Record<string, string>)?.offer ?? ""); }
    if (!userId) {
      const { data: gen } = await admin.from("analytics_events").select("user_id, event_data").in("event_name", ["pix_generated", "funnel_v2_pix_generated"]).contains("event_data", { order_id: orderId }).limit(1);
      if (gen?.[0]) { userId = gen[0].user_id as string; offer = String((gen[0].event_data as Record<string, string>)?.offer ?? offer); }
    }
    if (!userId) {
      const ext = chkOut?.metadata?.externalId as string | undefined;
      if (ext?.includes(":")) { userId = ext.split(":")[0]; offer = offer || ext.split(":")[1]; }
    }
    if (!offer) {
      const ext = chkOut?.metadata?.externalId as string | undefined;
      offer = ext?.split(":")[1] === "downsell" ? "downsell" : "lifetime";
    }
    if (!userId) { logStep("No user resolved for order", { orderId: orderId.slice(0, 24) }); return jsonResponse({ received: true, warning: "no_user" }); }
    ...
    const payload = {
      ...
      payment_method: "pix", abacatepay_billing_id: orderId, customer_email: email, amount_cents: amountCents,
    ...
    logStep("Access granted", { userId, offer, orderId: orderId.slice(0, 24), jaLiberado });
    ...
    if (!jaLiberado) {
      ...
      await sendToUtmify(admin, { userId, email, orderId, offer, amountCents, name: displayName });
      await sendMetaCapi(admin, { userId, email, orderId, offer, amountCents });
      try {
    ...
    return jsonResponse({ received: true, error: "internal" }); // 200 pra Pagar.me não queimar tentativas
  }
});
