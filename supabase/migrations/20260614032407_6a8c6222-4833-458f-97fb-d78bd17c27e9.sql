...
BEGIN
  ...
  WITH d AS (DELETE FROM public.analytics_events WHERE true RETURNING 1)
    SELECT count(*) INTO n_events FROM d;
  WITH d AS (DELETE FROM public.module_analytics WHERE true RETURNING 1)
    SELECT count(*) INTO n_modules FROM d;
  WITH d AS (DELETE FROM public.user_activations WHERE true RETURNING 1)
    SELECT count(*) INTO n_activations FROM d;
  WITH d AS (DELETE FROM public.winback_attempts WHERE true RETURNING 1)
    SELECT count(*) INTO n_winback FROM d;
  WITH d AS (DELETE FROM public.cancel_attempts WHERE true RETURNING 1)
