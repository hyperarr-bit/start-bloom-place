...
export default function ComecarTeste() {
  ...
  const [step, setStep] = useState<Step>(() => {
    ...
    // v83.1: volta da demo real (Preview) cai no ritual de compromisso.
    if (s === "compromissos") return "compromissos";
  ...
  const montou = () => {
    ...
    /* v83.1 (dono, 28/08): a demo é o APP REAL — módulos de verdade com dados
     * de demonstração (Preview + barra de tour), entrando pelo módulo da
     * porta escolhida. O DemoCta do shell devolve em ?step=compromissos
     * ("quer organizar sua vida?" → Claro! → contrato → paywall). A remontagem
     * preserva area/hora via localStorage. Telemetria: o Preview já emite
     * funnel_view step=demo com o módulo visitado. */
    const modulo = ({ dinheiro: "financas", rotina: "rotina", corpo: "treino", metas: "desenvolvimento", tudo: "rotina" } as Record<string, string>)[area ?? "tudo"] ?? "rotina";
    trackEvent("demo_entrou", { area: area ?? "?", modulo, funil: FUNIL });
    navigate(`/preview/${modulo}?funnel=1&tour=vida`);
  };
   ...
   * "resultado" não é o diagnóstico emocional deles, e o pedido queimava a
   * janela de 90 dias do aparelho ANTES da pessoa virar pagante (os 23
   * compradores de 28/08 ficaram todos inelegíveis). Os pedidos agora moram
   * nos momentos de valor DENTRO do app: conta paga, retrospectiva,
   * sequência de hábito, extrato importado — ver src/lib/avaliacao.ts. */
