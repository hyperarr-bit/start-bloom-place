...
import { PaywallDia14 as PaywallFlow } from "../dia14/PaywallDia14";
...
/**
 * FUNIL /direto — o /inicio SEM A DEMO (15/08, teste do dono).
 *
 * Cópia deliberada do funil do dia 14, que continua rodando intocado em
 * /inicio faturando: A/B só vale se o controle não for mexido. Aqui muda o
 * que está em teste, e nada mais.
 *
 *   início → quiz → prova → gerando → seu plano → central → COMPROMISSO
 *   → cadastro → paywall → checkout Cakto
 *
 * O QUE MUDA E POR QUÊ:
 *
 * 1. A DEMO SAI. No app, 50 pessoas/dia morriam passeando nela (medido
 *    14/08) — sandbox dispersa o desejo que o quiz acabou de construir, e
 *    ela ainda joga a pessoa PRA FORA do funil (window.location pro
 *    /preview) pra depender de um CTA de volta. Nenhum dos funis que
 *    convertem no mercado (Cal AI, BitePal, Brainrot) tem passeio livre
 *    antes do preço.
 *
 * 2. ENTRA O COMPROMISSO. Sem a demo, a pessoa iria da central direto pro
 *    cadastro — e o paywall receberia gente fria. A tela de compromisso
 *    (projeção com data + "assumir meu objetivo") é o que os três
 *    concorrentes põem imediatamente antes do preço, e no app dela a fuga
 *    de <5s no paywall caiu de 26% pra 10% (medido 14/08). Ela substitui o
 *    papel de aquecimento que a demo fazia mal.
 *
 * 3. Prova social na espera do plano (padrão Cal AI) — a mesma alegação da
 *    home, nenhum número novo.
 *
...
function RadarResultScreen({ answers, area, onDone }: { answers: Record<string, string>; area: AreaKey; onDone: () => void }) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto text-center">
      ...
      <Button size="lg" className="w-full h-12 text-base" onClick={() => { trackEvent("funnel_click", { cta: "result", area , funil: FUNIL }); onDone(); }}>
...
/**
 * COMPROMISSO — o degrau que substitui a demo como aquecimento.
 *
 * É a peça que Cal AI, BitePal e Brainrot têm imediatamente antes do preço:
 * uma projeção pessoal COM DATA e um botão de compromisso ("Reach 50kg by 27
 * October" → "Commit to my goal"). No app do CORE ela derrubou a fuga de
 * menos de 5 segundos no paywall de 26% pra 10% (medido 14/08) — por isso
 * ela vem junto na saída da demo, e não depois.
 *
 * Tudo deriva da ÁREA e do quiz: a vitória escolhida vira a meta com data,
 * o marco de 30 dias fala a língua da área (dinheiro usa a âncora em R$ que
 * a própria pessoa estimou) e o acento é a cor da área.
 */
function CompromissoScreen({ area, answers, onAssumir }: {
  area: AreaKey;
  answers: Record<string, string>;
  onAssumir: () => void;
}) {
  const a = AREAS[area];
  useEffect(() => { trackEvent("funnel_view", { step: "compromisso", area, funil: FUNIL }); }, [area]);
  ...
  return (
    <div className="w-full max-w-sm mx-auto text-center">
      <div className="text-[11px] font-bold uppercase tracking-widest mb-3" style={{ color: a.color }}>
        O seu compromisso
      </div>
      <h2 className="text-[27px] font-bold tracking-tight leading-[1.15] mb-2">
      ...
      <Button
        ...
        onClick={() => {
          trackEvent("funnel_click", { cta: "compromisso_assumir", area, vitoria: answers.vitoria ?? "", funil: FUNIL });
          onAssumir();
        }}
      >
        Assumir meu objetivo <ArrowRight className="w-4 h-4" />
      </Button>
      <p className="text-xs text-muted-foreground mt-3">Seu plano começa hoje · Garantia de 7 dias</p>
    </div>
  );
}
...
function CentralScreen({ area, onOpen }: { area: AreaKey; onOpen: () => void }) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto text-center">
      ...
      {/* Sem demo, este CTA não pode mais prometer passeio. Ele agora entrega
          pro compromisso — e a microcopy fala do próximo passo real. */}
      <Button size="lg" className="w-full h-12 text-base" onClick={() => { trackEvent("funnel_click", { cta: "central_open", area, funil: FUNIL }); onOpen(); }}>
        Continuar <ArrowRight className="w-4 h-4" />
      </Button>
      <p className="text-xs text-muted-foreground mt-3">Falta pouco pro seu plano começar</p>
    </div>
  );
}
...
function ProvaSocialScreen({ onNext }: { onNext: () => void }) {
  useEffect(() => { trackEvent("funnel_view", { step: "prova_social" , funil: FUNIL }); }, []);
  return (
...
function ResultScreen({ answers, onDone }: { answers: Record<string, string>; onDone: () => void }) {
  ...
  return (
    <div className="w-full max-w-sm mx-auto text-center">
      ...
      <Button size="lg" className="w-full h-12 text-base" onClick={() => { trackEvent("funnel_click", { cta: "result" , funil: FUNIL }); onDone(); }}>
...
function SignupScreen({ onSession, onConfirm }: { onSession: () => void; onConfirm: (email: string) => void }) {
  ...
  useEffect(() => {
    if (inApp) trackEvent("funnel_view", { step: "signup_inapp_browser" , funil: FUNIL });
  }, [inApp]);
  ...
  const handleGoogle = async () => {
    ...
    trackEvent("funnel_click", { cta: "signup_google", inapp: inApp , funil: FUNIL });
  ...
  const handleForgotPassword = async () => {
    ...
    trackEvent("funnel_click", { cta: "signup_reset_password" , funil: FUNIL });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (existingAccount) {
      trackEvent("funnel_click", { cta: "existing_login_submit" , funil: FUNIL });
      ...
      trackEvent("funnel_click", { cta: "signup_success", via: "existing_login" , funil: FUNIL });
      setLoading(false);
      onSession();
      return;
    }

    trackEvent("funnel_click", { cta: "signup_submit" , funil: FUNIL });
    ...
    trackEvent("funnel_click", { cta: "signup_success", instant: !!session , funil: FUNIL });
  ...
  return (
    <div className="w-full max-w-sm mx-auto">
      ...
      {/* Prova social DENTRO da espera (padrão Cal AI: o loading deles carrega
          um depoimento junto dos checkmarks). Mesma alegação da home — nenhum
          número novo inventado aqui. */}
      <motion.div
            ...
            {step === "start" && (vitrine ? (
              ...
              <StartScreen
                onPick={(firstAnswer) => {
                  ...
                  setAnswers(first);
                  trackEvent("funnel_click", { cta: "start" , funil: FUNIL });
