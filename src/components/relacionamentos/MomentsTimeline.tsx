  ...
  return (
    <div className="space-y-3 mt-3">
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={addMoment} className="w-full bg-primary text-primary-foreground rounded-lg py-1.5 text-xs font-bold">Salvar</motion.button>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="relative">
        {sorted.length > 0 && (
          <motion.div
            className="absolute left-3 top-0 bottom-0 w-px bg-rose-400/30"
            initial={{ scaleY: 0 }}
            animate={{ scaleY: 1 }}
        ...
        <AnimatePresence mode="popLayout">
          {sorted.map((m, i) => (
            ...
            >
              <motion.div
                className="absolute left-1.5 top-1 w-3 h-3 rounded-full bg-rose-400 border-2 border-background"
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
              ...
              <div className="bg-card rounded-xl border border-border p-3 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  ...
                  <motion.button whileTap={{ scale: 0.8 }} onClick={() => removeMoment(m.id)} className="text-muted-foreground hover:text-destructive p-1 transition-colors">
