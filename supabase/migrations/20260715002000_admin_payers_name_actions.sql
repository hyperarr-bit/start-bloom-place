-- 15/07: enriquece Pagantes com (a) NOME REAL do cadastro — está em
-- auth.users.raw_user_meta_data (display_name/full_name), o signup grava lá
-- mas nada copia pro profiles; (b) AÇÕES DE ATIVAÇÃO (key_action_completed:
-- first_income, first_bill, first_workout…) = o que a pessoa configurou;
-- (c) dias ativos distintos (retenção); (d) valor pago (lifetime vs downsell).
...
BEGIN
  ...
  mod_usage AS (
    SELECT user_id, module_id, SUM(duration_seconds) AS seconds, COUNT(*) AS opens
