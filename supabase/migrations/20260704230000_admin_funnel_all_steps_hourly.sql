-- Pedido do dono: mostrar TODAS as etapas literais do funil (não colapsar o
-- quiz em 1 linha, incluir a tela de preparação) + granularidade dia/hora na
-- série temporal (pra acompanhar campanha no mesmo dia, hora a hora).

-- A assinatura ganhou um 3º parâmetro (_granularity) — remove a versão de
-- 2 parâmetros primeiro pra não criar overload ambíguo no PostgREST.
DROP FUNCTION IF EXISTS public.admin_acquisition_funnel(timestamptz, timestamptz);
