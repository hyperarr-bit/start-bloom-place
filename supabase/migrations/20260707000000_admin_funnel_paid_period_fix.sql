-- Bug reportado pelo dono: o admin mostrava "N assinantes" num dia em que
-- ninguém pagou, porque `paid_users` contava QUALQUER usuário que já pagou
-- alguma vez (sem limite de data) — bastava ele abrir o app dentro do
-- período consultado pra ser recontado como "pagou hoje". Fix: o evento de
-- pagamento (trial_converted/subscription_started) também precisa ter
-- ocorrido dentro de [_from, _to).
...
BEGIN
  ...
  paid_users AS (
    -- FIX: exige que o próprio evento de pagamento tenha ocorrido dentro do
    -- período consultado — não basta o usuário ter pagado em qualquer data.
